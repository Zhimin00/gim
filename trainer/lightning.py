...
import sys
import pdb
mast3r_path = os.path.abspath("/home/cpeng26/scratchrchella4/mast3r-roma/mast3r")
dust3r_path = os.path.abspath("/home/cpeng26/scratchrchella4/mast3r-roma/mast3r/dust3r")
...
def fine_matching(query_views, map_views, model, device, max_batch_size, fast_nn_params):
    ...
    for ppi, (pp1, pp2, cc11, cc21) in enumerate(zip(descs1, descs2, confs1, confs2)):
        ...
        matches_im_map_ppi, matches_im_query_ppi = fast_reciprocal_NNs(pp2, pp1, subsample_or_initxy1=2,
                                                                       **fast_nn_params)
...
def coarse_coordinates(desc1, desc2, conf_list, hw0_i, hw1_i, HW0, HW1, fast_nn_params, subsample_or_initxy1=8):
    ...
    kpts0 = (
        np.stack(
            (
                (HW0[1] / hw0_i[1]) * (pts0[..., 0]),
                (HW0[0] / hw0_i[0]) * (pts0[..., 1]),
            ),
            axis=-1,
        )
    )
    kpts1 = (
        np.stack(
            (
                (HW1[1] / hw1_i[1]) * (pts1[..., 0]),
                (HW1[0] / hw1_i[0]) * (pts1[..., 1]),
            ),
            axis=-1,
        )
    )
...
class Trainer(pl.LightningModule):
    ...
    def __init__(self, pcfg, tcfg, dcfg, ncfg):
        ...
        elif pcfg.weight == 'mast3r_onlymega':
            detector = None
            model_path = "/home/cpeng26/scratchrchella4/checkpoints/mast3r_megadepth_0421/checkpoint-last.pth"
            model = AsymmetricMASt3R.from_pretrained(model_path) 
        elif pcfg.weight == 'mast3r_finetune':
            detector = None
            model_path = "/home/cpeng26/scratchrchella4/checkpoints/finetune/checkpoint-best.pth"
            model = AsymmetricMASt3R.from_pretrained(model_path) 
        elif pcfg.weight == 'mast3r_onlywarp':
            detector = None
            model_path = '/home/cpeng26/scratchrchella4/checkpoints/MASt3R_ViTLarge_BaseDecoder_512_onlywarp_0417/checkpoint-best.pth'
            model = AsymmetricMASt3R_only_warp.from_pretrained(model_path)
        elif pcfg.weight == 'mast3r_warpdpt':
            detector = None
            model_path = '/home/cpeng26/scratchrchella4/checkpoints/MASt3R_ViTLarge_BaseDecoder_512_warpdpt_0418/checkpoint-last.pth'
    ...
    def inference(self, data):
        ...
        elif self.pcfg.weight == 'mast3r' or self.pcfg.weight == 'mast3r_finetune' or self.pcfg.weight == 'aerial_mast3r':
            self.mast3r_inference2(data)
    ...
    def mast3r_inference(self, data):
        if data.get('color_fine0') is None:
            self.mast3r_inference1(data)
        else:
            self.mast3r_inference2(data)

    def mast3r_inference1(self, data):
        batch = [({"img": data['color0'], "idx": 0, "instance": 0},
                 {"img": data['color1'], "idx": 1, "instance": 1})]
        ...
        kpts0 = (
            torch.stack(
                (
                    (width0 / hw0_i[1]) * (pts0[..., 0]),
                    (height0 / hw0_i[0]) * (pts0[..., 1]),
                ),
                dim=-1,
            )
            + offset
        ...
        kpts1 = (
            torch.stack(
                (
                    (width1 / hw1_i[1]) * (pts1[..., 0]),
                    (height1 / hw1_i[0]) * (pts1[..., 1]),
                ),
                dim=-1,
            )
            + offset
    ...
    def mast3r_inference2(self, data):
        batch = [({"img": data['color0'], "idx": 0, "instance": 0},
                 {"img": data['color1'], "idx": 1, "instance": 1})]
        view1, view2 = collate_with_cat(batch[:1])
        pred1, pred2 = self.model(view1, view2)
        ...
        kpts0, kpts1, mconf = coarse_coordinates(desc1, desc2, conf_list, hw0_i, hw1_i, HW0, HW1, fast_nn_params, subsample_or_initxy1=8)
        ...
        if len(crops1) == 0 or len(crops2) == 0:
            kpts0, kpts1, mconf = coarse_coordinates(desc1, desc2, conf_list, hw0_i, hw1_i, HW0, HW1, fast_nn_params, subsample_or_initxy1=2)
            matches_im0 = torch.from_numpy(kpts0.copy()).float().to(device)
            matches_im1 = torch.from_numpy(kpts1.copy()).float().to(device)
        else:
