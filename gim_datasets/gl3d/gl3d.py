...
from gim_datasets.utils import read_images, read_images_two_sizes
...
class GL3DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        color_fine0, color_fine1 = None, None
...
class GL3DDataset2(Dataset):
    def __init__(self,
                 ...
                 max_overlap_score,
                 max_resize,        # max edge after resize
                 fine_resize,        # max edge after resize in coarse-to-fine matching
                 df,                # general is 8 for ResNet w/ pre 3-layers
                 padding,           # padding image for batch training
                 augment_fn,        # augmentation function
                 max_samples,       # max sample in current sequence
    ...
    def __getitem__(self, idx):
        ...
        width0, height0 = imagesize.get(img_path0)
        width1, height1 = imagesize.get(img_path1)
        if self.fine_resize is not None:
            image0, color0, scale0, resize0, mask0, color_fine0 = read_images_two_sizes(
            img_path0, self.max_resize, self.fine_resize, self.df, self.padding, None)
            image1, color1, scale1, resize1, mask1, color_fine1 = read_images_two_sizes(
                img_path1, self.max_resize, self.fine_resize, self.df, self.padding, None)
        else:
            image0, color0, scale0, resize0, mask0 = read_images(
                img_path0, self.max_resize, self.df, self.padding, None)
            image1, color1, scale1, resize1, mask1 = read_images(
